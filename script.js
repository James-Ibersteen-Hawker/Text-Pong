      ...
      class Puck {
        ...
        check() {
          if (
            ...
            if (self.grid[y][x] != self.glyphs.controller)
  ...
  async Open() {
    ...
    try {
      ...
      if (self.scores.lScore != self.scores.rScore) {
        const winner =
          Object.entries(self.players).find(
            (p) =>
              self.scores[
                Object.keys(self.scores).find(
                  (v) => v == [p[0], "Score"].join("") || v == ["Score", p[0]]
                )
              ] == Math.max(self.scores.lScore, self.scores.rScore)
          )?.[1] || "Error";
        await opening.Type(
